...
SYSTEM_PROMPT_EXTRACT = (
    ...
    "rationale.\n"
    "NEVER compute prices, weighted averages, or any other numbers yourself. "
    "When citing dollar figures or percentages, ONLY use exact values from "
    "PRE-COMPUTED FINDINGS or extracted_terms -- never invent or derive new "
    "numbers (no averaging tier prices, no projecting growth, no adjusting "
    "for inflation). If a needed number is not in the source data, say so "
    "explicitly rather than fabricating it.\n\n"
    "GROUNDING_SOURCES FORMATTING:\n"
    "Populate the grounding_sources array with 4-8 records you actually "
    "used. Each 'record' must read as a complete, business-friendly fact "
    "with units (no JSON-like field names, no debug strings). Each "
    "'used_for' must be a brief plain-English purpose. Examples of GOOD "
    "records:\n"
    "  - 'AlphaBI 3-year TCO of $3.56M ($177K under FY-25-27 envelope)'\n"
    "  - 'Industry benchmark: median price/user/year of $250'\n"
    "  - 'BeaconIQ uptime SLA 99.5% sits at the P10 floor'\n"
    "Examples of BAD records (do NOT produce these):\n"
    "  - 'PRE-COMPUTED FINDINGS: total_3yr_tco 3562800.72'\n"
    "  - 'budget total_3yr_delta 177199.28'\n\n"
...
SYSTEM_PROMPT_COMPARE = (
    ...
    "SCORE SCALE (CRITICAL):\n"
    "All weighted supplier scores in this report are on a 0-100 scale "
    "(computed as the weighted sum of dimension scores using the weights "
    "above, where each dimension is 0-10 and weights sum to 100). When "
    "you reference any supplier's overall score in your "
    "recommendation_rationale or executive_summary, ALWAYS use the 0-100 "
    "scale (e.g., 'AlphaBI scored 58.5 of 100'). Do NOT mix in the 0-10 "
    "per-dimension scale for overall scoring -- that scale is only for "
    ...
    "section. DO NOT recompute them. Reference them directly in your "
    "rationale.\n"
    "NEVER compute prices, weighted averages, or any other numbers "
    "yourself. When citing dollar figures or percentages, ONLY use exact "
    "values from PRE-COMPUTED FINDINGS or extracted_terms -- never invent "
    "or derive new numbers (no averaging tier prices, no projecting growth, "
    "no adjusting for inflation). For per-tier prices, cite the exact tier "
    "value (for example AlphaBI Tier-2 at $247/user/year, not $229.54).\n\n"
    "GROUNDING_SOURCES FORMATTING:\n"
    "Populate the grounding_sources array with 4-8 records you actually "
    "used. Each 'record' must read as a complete, business-friendly fact "
    "with units (no JSON-like field names, no debug strings). Each "
    ...
    "  - 'AlphaBI 3-year TCO of $3.56M ($177K under FY-25-27 envelope)'\n"
    "  - 'Industry benchmark: median price/user/year of $250'\n"
    "  - 'INT-SSO and INT-LDS dependencies (high criticality, Q2-Q3 2024)'\n"
    "Examples of BAD records (do NOT produce these):\n"
    "  - 'PRE-COMPUTED FINDINGS: AlphaBI total_3yr_tco 3562800.72'\n"
    "  - 'budget total_3yr_delta 177199.28 under budget'\n\n"
    "OUTPUT: Return ONLY valid JSON matching the schema below. No markdown, "
    "no preamble."
)
